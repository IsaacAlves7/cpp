/*BP45SC.C-SUBROUTINE FROM HYPERSIGNAL
**************************************************************************
...
double out_val[IMPULSE_LENGTH];
int   n;

    out_val[0]=filt(10000.0);  /* the "impulse" input */

    for (n=1; n<IMPULSE_LENGTH; n++)
    {
         out_val[n]=filt(0.0); /* zero input for rest of impulse response length */
    }
